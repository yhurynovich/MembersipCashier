...
namespace SecurityUnified.Serialization.Expressions.Parser
{
    public class CodeParser
    {
        ...
        private bool Read(bool isBuildDefineString, bool isIgnoreWhiteSpace)
        {
            ...
            // Trying to read after end then return 0
            ...
            // Check the whitespace characters, skip, continue
            ...
            if (char.IsLetter(c) || c == '_' || c == '$')
            {
                ...
                for (this.Length = 1; (this.Length + this.Index) < this.Content.Length; this.Length++)
                {
                    ...
                    // Back when the char instead of letters, numbers, underscores
            ...
            if (char.IsDigit(c))
            {
                ...
                for (this.Length = 1; (this.Length + this.Index) < this.Content.Length; this.Length++)
                {
                    ...
                    // When char is a point, it is determined whether the character behind the numbers, if it is digital, then return
                    if (cInner == '.')
                    {
                    ...
                    // Back when the char is not a number, mdflx (l represents a variety of digital type as represented long) time
            ...
            {
                // To the end of the direct return
                return true;
            }

            // If known symbols, do some processing
            switch (c)
            {
                #region case .....
                case '.':
                ...
                case '=':
                    ...
                    break;
               case '~':
                    if (nextInner == '=') // ~= (for like operator)
                    {
                        this.Length++;
                    }
                    break;
                ...
                case '/':
                    ...
                    else if (nextInner == '*') // Comment symbol:/**/
                    {
                    ...
                    if (nextInner == '\"') // @""
                    {
                        ...
                        for (int i = this.Index + this.Length; i < this.Content.Length; i++)
                        {
                            ...
                            if (this.Content[i] == '\"')
                            {
                                ...
                                if (isBuildDefineString)
                                {
                                    // Currently first with a replacement, free of optimization can be done
            ...
            // Processing may be followed by an equal sign (=) of
            #region switch (c)
            switch (c)
            {
                case '&':
